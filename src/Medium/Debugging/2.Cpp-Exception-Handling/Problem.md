...
3. Otherwise, if the call raises a `std::invalid_argument` exception, it has to print
...
To keep the code quality high, you are advised to have exactly one line printing
`returning control flow to caller` in the body of `process_input` function.
...
In the only line of the input, there is a single integer $n$, which is going to be 
the argument passed to function `process_input`.
...
In the first sample, $n$, so the call `largest_proper_divisor(0)` raises an exception. 
In this case, the function `process_input` prints two lines. 
In the first of them it prints the string representation of the raised exception, 
...
In the first sample, $n = 9$, so the call `largest_proper_divisor(9)` doesn't raise an 
exception and returns value $3$. In this case, the function `process_input` prints two lines.
In the first of them it prints `result=3` because the returned value by `largest_proper_divisor(9)`
is $3$, and in the second line it prints `returning control flow to caller`.