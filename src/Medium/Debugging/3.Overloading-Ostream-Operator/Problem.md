...
In the only line of the input there are $3$ space-separated strings `first_name`, `last_name`, `event`.  
The values of `first_name` and `last_name` will be used to create an object `p` of type **Person**.  
...
This code will use the implementation of **Person** public methods 
and the overloaded $<<$ operator to produce the output.  
